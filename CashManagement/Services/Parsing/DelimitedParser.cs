...
/// 編碼依 profile.Encoding（UTF-8 / BIG5）由 EncodingHelper 決定。
/// </summary>
public static class DelimitedParser
{
    public static List<RawRow> Parse(byte[] file, ParsedProfile profile)
    {
        var text = EncodingHelper.Resolve(profile.Encoding).GetString(file);
...
public static class ParserRegistry
{
    public static Func<byte[], ParsedProfile, List<RawRow>> GetParser(string engine) => engine switch
    {
        ...
        ParserEngines.FixedWidth => FixedWidthParser.Parse,
        _ => throw new BusinessException($"解析引擎 {engine} 尚未支援（目前支援 DELIMITED、FIXED_WIDTH）"),
    };
}
